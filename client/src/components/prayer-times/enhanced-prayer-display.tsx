...
export interface EnhancedPrayerDisplayProps {
  latitude?: number;
  longitude?: number;
  timezone?: number;
  className?: string;
...
export default function EnhancedPrayerDisplay({
  ...
  useEffect(() => {
    ...
    try {
      const config: PrayerTimesConfig = {
        calculationMethod: prayerConfig.calculationMethod || "MWL",
        asrJuristic: prayerConfig.asrJuristic || "Standard",
        adjustHighLats: prayerConfig.adjustHighLats || "AngleBased",
        timeFormat: prayerConfig.timeFormat || "24h"
      };
